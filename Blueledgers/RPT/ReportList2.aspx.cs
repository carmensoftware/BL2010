...
{
    public partial class ReportList2 : BasePage
    {
        protected IEnumerable<ReportView> _Reports;

        protected override void Page_Load(object sender, EventArgs e)
        {
            var group = Request.QueryString["group"] == null ? "ALL" : Request.QueryString["group"].ToString();

            _Reports = GetReport(group);
        }
        ...
        private IEnumerable<ReportView> GetReport(string group)
        {
            ...
            var query = @"
                            ...
                            WHERE 
                                IsActive=1 
                                AND [Group] <> 'FORM' 
                                AND [Group] =  CASE WHEN @group = 'ALL' THEN [Group] ELSE @group END
                            ORDER BY 
                                [Name]";
            var dt = ExecuteQuery(connectionString, query, new SqlParameter[] { new SqlParameter("group", group) });
