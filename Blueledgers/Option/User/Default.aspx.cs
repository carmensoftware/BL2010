...
namespace BlueLedger.PL.Option.User
{
    public partial class Default : BaseClass.BasePage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var changedDate = dlgPassword.GetLastUpdatedPassword(LoginInfo.LoginName);


                if (!string.IsNullOrEmpty(changedDate.ToString()))
                {
                    lbl_LastChangedPassword.Text = string.Format("<b>Password changed: </b>{0}<br/>", Convert.ToDateTime(changedDate).ToString("dd/MM/yyyy HH:mm:ss"));
                }
        ...
        protected void btn_ChangePassword_Click(object sender, EventArgs e)
        {
            ...
            dlgPassword.ChangePassword(loginName, "Change Password");
