...
public partial class StoredProcedures
{
    ...
    public static void PC_Cn_GetNewID()
    {
        using (SqlConnection connection = new SqlConnection("context connection = true"))
        {
            ...
            string prefixSelComm = "SELECT 'CN' + (substring(cast(year(getdate())as varchar(4)),3,2)"
            ...
            string runningNoSelCmd = "SELECT ISNULL(CAST(CAST(SUBSTRING(MAX(CnNo),10,4) AS INT) + 1 AS NVARCHAR),'') " +
                "FROM PC.Cn WHERE CnNo LIKE '" + preFix + "%'";
