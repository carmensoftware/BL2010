...
{
    public partial class ListPage : BaseUserControl
    {
        #region "Attributes"
        ...
        public override void DataBind()
        {
            ...
            {
                btn_ViewModify.Enabled = !viewHandler.GetIsStandard(int.Parse(ddl_View.Value.ToString()), LoginInfo.ConnStr);
            }
            ...
            this.grd_DataList.SettingsDetail.AllowOnlyOneMasterRowExpanded = false;
            // Display data list
            this.RefreshList();
        ...
        private void RefreshList()
        {
            ...
            if (result)
            {
                ...
                int wfId = viewHandler.GetWFId(int.Parse(ddl_View.Value.ToString()), LoginInfo.ConnStr);

                if (LoginInfo.BuInfo.IsHQ)
                {
                    for (int i = (viewHandler.GetIsWFEnable(int.Parse(ddl_View.Value.ToString()), LoginInfo.ConnStr) ? 3 : 2); 
                        i < dsDataList.Tables[PageCode].Columns.Count; i++)
                    {
                        GridViewDataColumn column   = new GridViewDataColumn();
                        string columnName           = dsDataList.Tables[PageCode].Columns[i].ColumnName;
                        double columnWidth          = viewHandlerCols.GetColumnWidth(int.Parse(ddl_View.Value.ToString()), columnName, LoginInfo.ConnStr);
                        ...
                        if (columnWidth != 0)
                        {
                            column.Width = Unit.Percentage(columnWidth);
                        }
                ...
                {
                    for (int i = (viewHandler.GetIsWFEnable(int.Parse(ddl_View.Value.ToString()), LoginInfo.ConnStr) ? 4 : 3); 
                        i < dsDataList.Tables[PageCode].Columns.Count; i++)
                    {
                        GridViewDataColumn column   = new GridViewDataColumn();
                        string columnName           = dsDataList.Tables[PageCode].Columns[i].ColumnName;
